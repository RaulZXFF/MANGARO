(function () {
  ...
  const storedZoomValue = parseFloat(localStorage.getItem(zoomPreferenceKey));
  let zoom = Number.isFinite(storedZoomValue) ? storedZoomValue : 1;

  let theme = localStorage.getItem(themePreferenceKey) || 'theme-dark';
    ...
    if (page === total) {
      ...
      if (slug) {
        ...
        if (!read.includes(slug)) {
          read.push(slug);
          localStorage.setItem(readChaptersKey, JSON.stringify(read));
        }
      }
      document.dispatchEvent(
  ...
  function setZoom(nextValue, opts = {}) {
    ...
    localStorage.setItem(zoomPreferenceKey, String(zoom));
    if (!opts.silent) {
      document.dispatchEvent(
